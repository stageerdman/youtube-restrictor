...
Version: `0.2.0` — implemented by `native-host/`, `shared/messaging/`
...
`HeartbeatMonitor` tracks the last one received per browser; if a given
browser is running and none has arrived from it for 5 minutes,
`EnforcementController` quits it via that browser's own enforcer
(`FirefoxEnforcer`/`SafariEnforcer`/`ChromeEnforcer`) — the menu bar app
only needs to know "extension is alive", not "video is playing".

```json
{ "type": "heartbeat", "version": "0.2.0", "timestamp": 1234567890000 }
```

**`source` is added by `native-host/host.js`, not the extension.**
Firefox, Chrome, and Brave all send the exact JSON body above — the
extension itself has no notion of "which browser am I" to stamp into
it. `native-host/host.js` is spawned via a per-browser wrapper script
(`native-host/manifest/run-host-firefox.sh` /
`run-host-chrome.sh` / `run-host-brave.sh`, generated by
`scripts/install-native-host*.sh` from
`native-host/manifest/run-host.template.sh`) that passes
`--source=firefox`/`chrome`/`brave` ahead of the browser's own args;
host.js reads that and adds a `source` field to every message before
forwarding it into the socket to the menu bar app:

```json
{ "type": "heartbeat", "version": "0.2.0", "timestamp": 1234567890000, "source": "brave" }
```

Without this, the menu bar app has no way to tell a live Chrome
heartbeat apart from a dead Firefox one — both arrive over the same
socket in the same shape — which used to mean one browser's live
heartbeat could mask another's going silent. Safari doesn't need this:
its relay (below) is a transport only Safari ever speaks to, so the app
tags it `"safari"` itself rather than trusting the client.
